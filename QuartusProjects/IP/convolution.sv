...
(
    ...
    always_ff @ (posedge clock) begin
        ...
        if (k_st_ready_reg & k_st_valid) begin
            for (y=KY-1; y>=0; y--) begin
                for (x=KX-1; x>=0; x--) begin
                    if (x == 0) begin
                        if (y == 0) begin
    ...
    always_comb begin
        for (y=0; y<KY-1; y++) begin
            for (t=0; t<TAPS; t++) begin
                buffer_taps[t][y] = buffer[XRES_TAPS[t]-1][y];
            end
        end
    end
    always_ff @ (posedge clock) begin
        pixel_valid <= ((count_x >= PAD) && (count_x <= (XRES_TAPS[select_reg] - PAD - 1))) &&
            ((count_y >= PAD) && (count_y <= (XRES_TAPS[select_reg] - PAD - 1)));
